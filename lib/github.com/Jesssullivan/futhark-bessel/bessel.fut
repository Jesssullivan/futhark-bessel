...
module f64 = {
  ...
  def sincos_reduced (phase: f64) : (f64, f64) =
    ...
    let q = ((n % 4) + 4) % 4
    in if q == 0 then (sin_r, cos_r)
       else if q == 1 then (cos_r, -sin_r)
       else if q == 2 then (-sin_r, -cos_r)
       else (-cos_r, sin_r)
  ...
  def hankel_sums (nu: f64) (x: f64) : (f64, f64) =
    let (_, _, p, q) =
      loop (a, invpow, p, q) = (1.0, 1.0, 1.0, 0.0) for k < 12 do
        let m = k + 1
...
module f32 = {
  ...
  def hankel_sums (nu: f32) (x: f32) : (f32, f32) =
    let (_, _, p, q) =
      loop (a, invpow, p, q) = (1.0, 1.0, 1.0, 0.0) for k < 7 do
        let m = k + 1
